...
async def test_database_contents(db_path: str, database: str) -> bool:
    ...
    try:
        ...
        # Security: Use allowlist to prevent injection even if table names become dynamic
        ALLOWED_TABLES = {"Episodic", "Entity", "Community"}
        tables_to_check = ["Episodic", "Entity", "Community"]

        for table in tables_to_check:
            # Security: Validate table name against allowlist before use
            if table not in ALLOWED_TABLES:
                print(f"    {table}: (skipped - not in allowlist)")
                continue
            try:
                # Security: Table name is validated against allowlist above
                # Note: Cypher doesn't support parameterized labels, so we validate instead
