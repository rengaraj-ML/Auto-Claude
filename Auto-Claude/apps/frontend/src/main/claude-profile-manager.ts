  ...
  getActiveProfileEnv(): Record<string, string> {
    ...
    if (profile?.configDir) {
      ...
      if (credentials.token) {
        ...
        // If Keychain lookup fails, try to use token from environment variable (.env file)
        // This is a fallback for when Credential Manager times out
        const envToken = process.env.CLAUDE_CODE_OAUTH_TOKEN;
        if (envToken) {
          env.CLAUDE_CODE_OAUTH_TOKEN = envToken;
          debugLog('[ClaudeProfileManager] Using CLAUDE_CODE_OAUTH_TOKEN from environment (.env file) for profile:', profile.name);
        } else {
          debugLog(
            '[ClaudeProfileManager] No token found in Keychain or environment for profile without configDir:',
            profile.name,
